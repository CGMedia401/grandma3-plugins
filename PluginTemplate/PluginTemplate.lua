...
--[[
=====================================================================
 PluginTemplate.lua - Starting point for new CG Media & Lites plugins
=====================================================================
 HOW TO USE THIS TEMPLATE
 1. Copy this whole folder, rename it (and the .lua/.xml files inside)
    to your new plugin's name, e.g. "MyNewPlugin".
 2. Find/replace every "PluginTemplate" in this file with your new
    plugin's actual name (the texture namespace, window name, and
    Printf debug prefixes all use it - keeping each plugin's texture
    names unique is what lets multiple plugins run in the same
    session without colliding).
 3. Update the .xml descriptor's Name/Path/ComponentLua Name/FileName
    to match too (see GroupCleanupV4.xml or SaveShowAs3.xml for the
    two-file xml+lua format).
 4. Replace the placeholder content area in Main() (search "YOUR
    CONTENT HERE" below) with whatever the plugin actually needs.
 5. Test the .lua's syntax before deploying: `luac -p YourFile.lua`
    (install once via `brew install lua` if this Mac doesn't have it).

 WHAT'S ALREADY WIRED UP, PROVEN LIVE ACROSS GroupCleanupV4 AND
 SaveShowAs3 (Mac onPC, Windows onPC, both confirmed working):
 - Self-installing CG Media & Lites logo: embedded as base64 below,
   written to disk on first run on whatever machine this runs on, no
   manual per-machine texture copy needed. See project_grandma3_plugins
   memory for the full history of gotchas already solved here (the
   GetPath() no-trailing-slash bug, re-applying name/fileName on every
   run, etc.) - don't re-derive these, they're already fixed.
 - CG-branded custom window: BaseInput + TitleBar (with the logo) +
   CloseButton + DialogFrame, matching both existing plugins' look.
 - A single working content/button layout (DialogFrame Rows=2: a
   Stretch content area on top, a Fixed-height button row on the
   bottom) - this exact shape is proven live multiple times. Multi-
   COLUMN layouts (e.g. two buttons side by side) are NOT reliably
   solved yet - see the "even-width buttons" saga in the project
   memory before attempting one; consider stacking buttons vertically
   instead if you need more than one action, or reuse a working
   reference (GroupCleanupV4's checkbox rows) rather than guessing at
   DialogFrame's own multi-column cell indexing again.
=====================================================================
]]--

local EMBEDDED_TEXTURES = {
    ['PluginTemplate/cg_logo.tga'] = (
...
-- Writes relativeFileName (e.g. 'PluginTemplate/cg_logo.tga') under
...
local function ensureTextureFileOnDisk(relativeFileName)
    ...
    if not okPath or texturesRoot == nil then
        Printf('PluginTemplate asset debug: GetPath(Enums.PathType.Textures) failed err=%s', tostring(texturesRoot))
        return false
    end
    ...
    if data == nil then
        Printf('PluginTemplate asset debug: no embedded data for %s', relativeFileName)
        return false
    end

    local okDecode, decoded = pcall(base64Decode, data)
    if not okDecode or decoded == nil then
        Printf('PluginTemplate asset debug: base64Decode failed for %s err=%s', relativeFileName, tostring(decoded))
        return false
    end

    local okOpen, f = pcall(io.open, fullPath, 'wb')
    if not okOpen or f == nil then
        Printf('PluginTemplate asset debug: could not open %s for writing', fullPath)
        return false
    end

    f:write(decoded)
    f:close()
    Printf('PluginTemplate asset debug: wrote %s (%d bytes)', fullPath, #decoded)
    return true
end

local LOGO_TEXTURE_NAME = 'PluginTemplate_Logo'
local LOGO_TEXTURE_FILE = 'PluginTemplate/cg_logo.tga'
...
local function ensureTexture(textureName, fileName)
    ...
    if not okTextures or textures == nil then
        Printf('PluginTemplate texture debug: could not access TextureCollect.Textures ok=%s', tostring(okTextures))
        return nil
    end
    ...
    if tex == nil then
        ...
        if not okAppend or newTex == nil then
            Printf('PluginTemplate texture debug: textures:Append(Texture) failed for %s ok=%s val=%s', textureName, tostring(okAppend), tostring(newTex))
            return nil
        end
        tex = newTex
    end
    ...
    if not okSet then
        Printf('PluginTemplate texture debug: setting name/fileName failed for %s err=%s', textureName, tostring(setErr))
        return nil
    end
...
local function Main(display_handle, arguments)
    ...
    baseInput.Name = 'PluginTemplateWindow'
    baseInput.H = 0
    baseInput.W = 500
    baseInput.Columns = 1
    baseInput.Rows = 2
    ...
    titleBarIcon.Text = 'Plugin Template'  -- TODO: your window title
    ...
    titleBarCloseButton.Clicked = 'PT_CloseClicked'
    ...
    -- ===============================================================
    -- YOUR CONTENT HERE - replace this placeholder with whatever the
    -- plugin actually needs (checkboxes, text, etc.), appended into
    -- dlgFrame the same way GroupCleanupV4's checkBoxGrid was.
    -- ===============================================================
    ...
    local placeholderText = contentArea:Append('Button')
    placeholderText.Anchors = '0,0'
    placeholderText.Text = 'Replace this content area'
    placeholderText.Font = 'Medium20'
    placeholderText.TextalignmentH = 'Centre'

    local applyButton = dlgFrame:Append('Button')
    applyButton.Anchors = '0,1'
    applyButton.Textshadow = 1
    applyButton.HasHover = 'Yes'
    applyButton.Text = 'Apply'  -- TODO: your action label
    applyButton.Font = 'Medium20'
    applyButton.TextalignmentH = 'Centre'
    applyButton.PluginComponent = my_handle
    applyButton.Clicked = 'PT_ApplyClicked'

    signalTable.PT_ApplyClicked = function(caller)
        GetFocusDisplay().ScreenOverlay:ClearUIChildren()
        continue = true
    end

    signalTable.PT_CloseClicked = function(caller)
        GetFocusDisplay().ScreenOverlay:ClearUIChildren()
        cancelled = true
        continue = true
    end
    ...
    if cancelled then
        Printf("Plugin Template: cancelled")
        return
    end

    -- TODO: your actual logic goes here, e.g. Cmd(...) calls.
    Printf("Plugin Template: applied")
end
