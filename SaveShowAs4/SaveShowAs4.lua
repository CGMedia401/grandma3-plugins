...
-- Writes relativeFileName (e.g. 'SaveShowAs4/cg_logo.tga') under
...
local function ensureTextureFileOnDisk(relativeFileName)
    ...
    if not okPath or texturesRoot == nil then
        Printf('SS4 asset debug: GetPath(Enums.PathType.Textures) failed err=%s', tostring(texturesRoot))
        return false
    end
    ...
    if data == nil then
        Printf('SS4 asset debug: no embedded data for %s', relativeFileName)
        return false
    end

    local okDecode, decoded = pcall(base64Decode, data)
    if not okDecode or decoded == nil then
        Printf('SS4 asset debug: base64Decode failed for %s err=%s', relativeFileName, tostring(decoded))
        return false
    end

    local okOpen, f = pcall(io.open, fullPath, 'wb')
    if not okOpen or f == nil then
        Printf('SS4 asset debug: could not open %s for writing', fullPath)
        return false
    end

    f:write(decoded)
    f:close()
    Printf('SS4 asset debug: wrote %s (%d bytes)', fullPath, #decoded)
    return true
end

local LOGO_TEXTURE_NAME = 'SaveShowAs4_Logo'
local LOGO_TEXTURE_FILE = 'SaveShowAs4/cg_logo.tga'
...
local function ensureTexture(textureName, fileName)
    ...
    if not okTextures or textures == nil then
        Printf('SS4 texture debug: could not access TextureCollect.Textures ok=%s', tostring(okTextures))
        return nil
    end
    ...
    if tex == nil then
        ...
        if not okAppend or newTex == nil then
            Printf('SS4 texture debug: textures:Append(Texture) failed for %s ok=%s val=%s', textureName, tostring(okAppend), tostring(newTex))
            return nil
        end
        tex = newTex
    end
    ...
    if not okSet then
        Printf('SS4 texture debug: setting name/fileName failed for %s err=%s', textureName, tostring(setErr))
        return nil
    end
...
-- USB backup
-- ===================================================================
-- After the normal save (whatever drive is currently selected - Drive 1
-- internal, on the console or onPC), mirror the show onto every USB
-- stick plugged in right now. ObjectList('Drive Thru') enumerates
-- connected drives (.no = drive number, .drivetype == 'Removeable' for
-- USB), and 'Select Drive N' + SaveShow is the documented way to target
-- one. Reselects Drive 1 afterward so the console is left as found.
local function saveShowToUsbDrives(fullName)
    local okList, drives = pcall(ObjectList, 'Drive Thru')
    if not okList or drives == nil then
        Printf('SS4 save debug: ObjectList(Drive Thru) failed err=%s', tostring(drives))
        return
    end
    ...
    for i = 1, #drives do
        ...
        if drive ~= nil and drive.drivetype == 'Removeable' and drive.no ~= nil then
            local okSelect = pcall(Cmd, string.format('Select Drive %s', tostring(drive.no)))
            if okSelect then
                local okSave = pcall(Cmd, string.format("saveshow '%s' /nc", fullName))
                if okSave then
                    savedAny = true
                    Printf("Show also saved to USB drive %s as '%s'", tostring(drive.no), fullName)
                else
                    Printf('SS4 save debug: saveshow failed on USB drive %s', tostring(drive.no))
                end
            else
                Printf('SS4 save debug: Select Drive %s failed', tostring(drive.no))
            end
        end
    end

    if savedAny then
        pcall(Cmd, 'Select Drive 1')
    end
end
...
local function Main(display_handle, arguments)
    ...
    if choice == "YES" then
        ...
        currentName = currentName:gsub("_%d%d%d%d$", "")

        local name = TextInput("Save Show As", currentName)
