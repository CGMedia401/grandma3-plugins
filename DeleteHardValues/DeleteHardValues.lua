...
--[[
=====================================================================
 DeleteHardValues.lua - built from PluginTemplate, 2026-09-02
=====================================================================
 GOAL (per user request): search every sequence for attribute values
 that are plain "hard values" (typed/stored directly into a cue, not
 linked to a preset, not driven by a recipe) and delete them.

 STATUS: SCAN / REPORT ONLY. This version does NOT delete anything.

 WHY: MA3's own manual confirms cues can hold three kinds of value -
 hard values, preset references, and recipes (see cue_sequence.html /
 presets_recipes.html / phaser_editor.html). But there is no
 documented command-line or Lua API call that deletes a single
 attribute's hard value while leaving recipes/presets on the same cue
 part untouched. Rather than guess at a destructive Cmd() sequence
 with no way to live-test it first, this version scans every
 sequence/cue/cue-part and writes a plain-text report of every hard
 value it finds, so it can be reviewed against a real show before any
 delete logic gets written and tested live.

 HOW THE DETECTION WORKS (confirmed from MA3's own bundled QA test
 suite at shared/resource/lib_plugins/systemtests/db/ - NOT guessed):
 - GetPresetData(cuePart, false, true) returns a table with
   .by_fixtures[fixtureId][attributeName] = a "phaser" entry.
 - A phaser entry has .abs_preset / .rel_preset set when that
   attribute's value is linked to a preset (see
   system_test_helping_functions_db.lua:311-320,
   CheckPhaserInternal reading ph.abs_preset / ph.rel_preset).
 - A phaser entry with NO abs_preset/rel_preset but real .steps data
   (.absolute / .relative / .integrated) is a plain hard value.
 - Recipes are separate child objects on the cue part itself
   (cuePart:Count() / cuePart:Ptr(n), see
   system_test_cuepart_recipe_properties.lua:16-19,
   part:Append("StandardRecipe")) - they are the SOURCE that cooks a
   value into the form above; this version reports the recipe-line
   count per cue part alongside each hard value found, so it can be
   sanity-checked against what's actually driving that part.
 GetPresetData() itself is documented at
 lua_objectfree_getpresetdata.html (Object-Free API, confirmed to
 accept preset/cue-part handles).

 NOT YET LIVE-TESTED - first real run should be on a disposable/backup
 show, with the System Monitor open, to confirm the report matches
 what the Sequence Sheet actually shows before trusting it on a real
 show.

 NEXT STEP (once the report is reviewed against a real show): work out
 and live-test the actual per-attribute delete mechanism, then wire up
 a real "Delete" action here. Don't add destructive Cmd() calls to
 this file without that live verification step - see the project
 memory's repeated lesson that a Cmd() call can report OK/return with
 no error while silently not doing what it should.
=====================================================================
]]--

local EMBEDDED_TEXTURES = {
    ['DeleteHardValues/cg_logo.tga'] = (
...
local function ensureTextureFileOnDisk(relativeFileName)
    ...
    if not okPath or texturesRoot == nil then
        Printf('DeleteHardValues asset debug: GetPath(Enums.PathType.Textures) failed err=%s', tostring(texturesRoot))
        return false
    end
    ...
    if data == nil then
        Printf('DeleteHardValues asset debug: no embedded data for %s', relativeFileName)
        return false
    end

    local okDecode, decoded = pcall(base64Decode, data)
    if not okDecode or decoded == nil then
        Printf('DeleteHardValues asset debug: base64Decode failed for %s err=%s', relativeFileName, tostring(decoded))
        return false
    end

    local okOpen, f = pcall(io.open, fullPath, 'wb')
    if not okOpen or f == nil then
        Printf('DeleteHardValues asset debug: could not open %s for writing', fullPath)
        return false
    end

    f:write(decoded)
    f:close()
    Printf('DeleteHardValues asset debug: wrote %s (%d bytes)', fullPath, #decoded)
    return true
end

local LOGO_TEXTURE_NAME = 'DeleteHardValues_Logo'
local LOGO_TEXTURE_FILE = 'DeleteHardValues/cg_logo.tga'
...
local function ensureTexture(textureName, fileName)
    ...
    if not okTextures or textures == nil then
        Printf('DeleteHardValues texture debug: could not access TextureCollect.Textures ok=%s', tostring(okTextures))
        return nil
    end
    ...
    if tex == nil then
        ...
        if not okAppend or newTex == nil then
            Printf('DeleteHardValues texture debug: textures:Append(Texture) failed for %s ok=%s val=%s', textureName, tostring(okAppend), tostring(newTex))
            return nil
        end
        tex = newTex
    end
    ...
    if not okSet then
        Printf('DeleteHardValues texture debug: setting name/fileName failed for %s err=%s', textureName, tostring(setErr))
        return nil
    end
...
-- Best-effort IsClass check. Returns true/false when the console can
-- answer, or nil when the call itself failed (unknown class name,
-- handle doesn't support IsClass, etc.) - callers decide how to treat
-- "unknown" since this is read-only and never destructive.
local function safeIsClass(obj, className)
    local ok, res = pcall(function() return obj:IsClass(className) end)
    if ok then return res end
    return nil
end

local function safeName(obj, fallback)
    local ok, name = pcall(function() return obj.Name end)
    if ok and name ~= nil and name ~= '' then return name end
    return fallback
end

-- Returns the by_fixtures table for a cue part, trying the documented
-- GetPresetData() call first and falling back to reading the property
-- directly off the handle (some MA3 object-model data is exposed as a
-- plain property without an explicit accessor call) - see the header
-- comment for why both paths are attempted rather than assumed.
local function getByFixtures(cuePart)
    local ok, data = pcall(GetPresetData, cuePart, false, true)
    if ok and data ~= nil and data.by_fixtures ~= nil then
        return data.by_fixtures, 'GetPresetData'
    end
    local ok2, direct = pcall(function() return cuePart.by_fixtures end)
    if ok2 and direct ~= nil then
        return direct, 'direct property'
    end
    return nil, nil
end

-- A hard value is a phaser entry with NO abs_preset/rel_preset link but
-- real stored step data. CORRECTED from a live report dump, 2026-09-03:
-- the step entries are direct numeric keys on the phaser table itself
-- (ph[1], ph[2], ...) alongside sibling fields like abs_preset/gridpos/
-- selective - there is NO separate ".steps" sub-table. The earlier
-- version checked ph.steps[i], which never existed, so this always
-- returned false regardless of real data - confirmed by a real report
-- showing e.g. ColorRGB_B = { 1 = { absolute = 100.0, ... }, gridpos =
-- 62, abs_preset = <handle:Ash_Lav>, ... } all as siblings.
local function isHardValuePhaser(ph)
    if type(ph) ~= 'table' then return false end
    if ph.abs_preset ~= nil or ph.rel_preset ~= nil then return false end
    for k, step in pairs(ph) do
        if type(k) == 'number' and type(step) == 'table' then
            if step.absolute ~= nil or step.relative ~= nil or step.integrated ~= nil or step.absolute_value ~= nil then
                return true
            end
        end
    end
...
-- Parses the user's sequence-selector text into either the string 'all'
-- or a list of selectors, each {kind='number', value=N} or
-- {kind='name', value="Some Sequence"}. Supports comma-separated lists
-- mixing numbers, "X Thru Y" / "X-Y" numeric ranges, and plain sequence
-- names (quoted or not) - e.g. "1,3,5 Thru 8,Position Phaser". A token
-- that isn't a pure number or a numeric range is treated as a name and
-- looked up directly (DataPool().Sequences[name] bracket lookup, same
-- as DataPool().Sequences["Root"] confirmed in MA3's own QA test suite
-- at system_test_ui_userinput.lua:32). Returns (nil, reason) on a
-- blank/cancelled input or on unparseable text - blank is treated as a
-- cancel, NOT as "all", so a scan never silently covers every sequence
-- unless the user explicitly typed "all".
local function parseSequenceSelector(input)
    if input == nil then return nil, 'cancelled' end
    local trimmed = input:match('^%s*(.-)%s*$')
    if trimmed == '' then return nil, 'cancelled' end
    if trimmed:lower() == 'all' then return 'all' end

    local selectors = {}
    for rawToken in trimmed:gmatch('[^,]+') do
        local token = rawToken:match('^%s*(.-)%s*$')
        if token ~= '' then
            local a, b = token:match('^(%d+)%s*[Tt][Hh][Rr][Uu]%s*(%d+)$')
            if not a then
                a, b = token:match('^(%d+)%s*%-%s*(%d+)$')
            end
            if a and b then
                a, b = tonumber(a), tonumber(b)
                if a > b then a, b = b, a end
                for n = a, b do
                    table.insert(selectors, {kind = 'number', value = n})
                end
            elseif token:match('^%d+$') then
                table.insert(selectors, {kind = 'number', value = tonumber(token)})
            else
                -- treat as a sequence name; strip optional surrounding quotes
                local name = token:match('^"(.*)"$') or token:match("^'(.*)'$") or token
                table.insert(selectors, {kind = 'name', value = name})
            end
        end
    end

    if #selectors == 0 then
        return nil, 'Could not find a sequence number or name in "' .. input .. '". Use a number (5), a range (1 Thru 5), a name (Position Phaser), a list of these, or "All".'
    end
    return selectors
end

-- Best-effort stringify for a single (non-table) value, including MA3
-- handles (light_userdata) - tries .Name then :ToAddr() before giving up.
local function safeToString(v)
    ...
    if t == 'userdata' or t == 'light_userdata' then
        local okName, name = pcall(function() return v.Name end)
        if okName and name ~= nil and name ~= '' then
            return '<handle:' .. tostring(name) .. '>'
        end
        local okAddr, addr = pcall(function() return v:ToAddr() end)
        if okAddr and addr ~= nil then
            return '<handle:' .. tostring(addr) .. '>'
        end
        return '<handle>'
    end
    return '<' .. tostring(t) .. '>'
...
-- Recursively dumps an arbitrary Lua table (bounded depth) to a plain
-- text block - used to see the REAL shape of GetPresetData's returned
-- data for cue parts that have recipes, since the hard-value detection
-- logic needs to be checked against real data rather than assumed from
-- documentation/test-code alone.
local function dumpTable(t, indent, maxDepth)
    indent = indent or ''
    maxDepth = maxDepth or 4
    local out = {}
    if maxDepth <= 0 then
        table.insert(out, indent .. '...(max depth reached)\n')
    ...
    local okIter, iterErr = pcall(function()
        for k, v in pairs(t) do
            ...
            if type(v) == 'table' then
                table.insert(out, indent .. kStr .. ' = {\n')
                table.insert(out, dumpTable(v, indent .. '  ', maxDepth - 1))
                table.insert(out, indent .. '}\n')
            else
                table.insert(out, indent .. kStr .. ' = ' .. safeToString(v) .. '\n')
            end
        end
    end)
    if not okIter then
        table.insert(out, indent .. '<error iterating table: ' .. tostring(iterErr) .. '>\n')
    end
    return table.concat(out)
end

-- Scans a single sequence handle into result. Shared by both the
-- "all sequences" and "specific sequence(s)" scan paths below.
local function scanOneSequence(seq, seqName, result)
    local okCueCount, cueCount = pcall(function() return seq:Count() end)
    if not okCueCount then return end

    for ci = 1, cueCount do
        local okCue, cue = pcall(function() return seq:Ptr(ci) end)
        if okCue and cue ~= nil then
            local isCue = safeIsClass(cue, 'Cue')
            if isCue == true then
                local cueName = safeName(cue, 'Cue index ' .. ci)
                local okPartCount, partCount = pcall(function() return cue:Count() end)
                local okCueNo, cueNo = pcall(function() return cue.No end)
                local okSeqAddr, seqAddr = pcall(function() return seq:ToAddr() end)
                if okPartCount then
                    result.cueCount = result.cueCount + 1
                    for pi = 1, partCount do
                        local okPart, part = pcall(function() return cue:Ptr(pi) end)
                        if okPart and part ~= nil then
                            result.partCount = result.partCount + 1
                            local okRecipeCount, recipeCount = pcall(function() return part:Count() end)
                            recipeCount = okRecipeCount and recipeCount or -1

                            local byFixtures, source = getByFixtures(part)
                            if byFixtures == nil then
                                table.insert(result.errors, string.format(
                                    '%s > %s > Part %d: could not read attribute data',
                                    seqName, cueName, pi - 1))
                            else
                                for fid, fixData in pairs(byFixtures) do
                                    if type(fixData) == 'table' then
                                        for attrName, ph in pairs(fixData) do
                                            if isHardValuePhaser(ph) then
                                                result.hardCount = result.hardCount + 1
                                                table.insert(result.lines, string.format(
                                                    '%s > %s > Part %d > Fixture %s > %s = HARD VALUE  (recipe lines on part: %d, read via %s)',
                                                    seqName, cueName, pi - 1, tostring(fid), tostring(attrName), recipeCount, source))

                                                -- Group into a batch delete unit keyed by
                                                -- (sequence, cue, part, attribute) so the
                                                -- delete step can select every matching
                                                -- fixture at once instead of one Cmd() per
                                                -- single value. Only groupable when we could
                                                -- read the cue's real number and the
                                                -- sequence's address - otherwise this entry
                                                -- is still reported above, just not
                                                -- deletable automatically.
                                                if okCueNo and cueNo ~= nil and okSeqAddr and seqAddr ~= nil then
                                                    local groupKey = seqAddr .. '|' .. tostring(cueNo) .. '|' .. tostring(pi - 1) .. '|' .. tostring(attrName)
                                                    local group = result.hardGroups[groupKey]
                                                    if group == nil then
                                                        group = {
                                                            seqAddr = seqAddr,
                                                            seqName = seqName,
                                                        ...
                                                        result.hardGroups[groupKey] = group
                                                        table.insert(result.hardGroupOrder, groupKey)
                                                    end
                                                    table.insert(group.fids, tostring(fid))
                                                else
                                                    table.insert(result.errors, string.format(
                                                        '%s > %s > Part %d > Fixture %s > %s: could not read cue number/sequence address - cannot auto-delete this one',
                                                        seqName, cueName, pi - 1, tostring(fid), tostring(attrName)))
                                                end
                                            end
                                        end
                                    end
...
-- Scans according to scope: the string 'all' (every sequence in the
-- pool), or a list of specific sequence numbers (from
-- parseSequenceSelector). Returns {lines, hardCount, cueCount,
-- partCount, errors, sequencesScanned}.
local function scanSequences(scope)
    local result = {
        lines = {}, hardCount = 0, cueCount = 0, partCount = 0, errors = {},
        sequencesScanned = {}, debugDumps = {},
        hardGroups = {}, hardGroupOrder = {},
    }

    local okSeqs, sequences = pcall(function() return DataPool().Sequences end)
    if not okSeqs or sequences == nil then
        table.insert(result.errors, 'Could not access DataPool().Sequences')
        return result
    end

    if scope == 'all' then
        local okCount, seqCount = pcall(function() return sequences:Count() end)
        if not okCount then
            table.insert(result.errors, 'Could not count DataPool().Sequences')
            return result
        end
        for si = 1, seqCount do
            local okSeq, seq = pcall(function() return sequences:Ptr(si) end)
            if okSeq and seq ~= nil then
                local seqName = safeName(seq, 'Sequence ' .. si)
                table.insert(result.sequencesScanned, seqName)
                scanOneSequence(seq, seqName, result)
            end
        end
    else
        for _, sel in ipairs(scope) do
            -- bracket lookup works for both a number key (Sequences[5])
            -- and a string name key (Sequences["Position Phaser"])
            local okSeq, seq = pcall(function() return sequences[sel.value] end)
            if okSeq and seq ~= nil then
                local seqName = safeName(seq, tostring(sel.value))
                table.insert(result.sequencesScanned, seqName)
                scanOneSequence(seq, seqName, result)
            else
                table.insert(result.errors, string.format('Sequence "%s" not found - skipped', tostring(sel.value)))
            end
        end
    end
...
-- cue.No is a scaled integer (e.g. 13100 = cue 13.1, confirmed from
-- MA3's own QA test suite, system_test_cue_copy.lua). Formats it back
-- into a clean command-line-friendly cue number string ("13.1", "4",
-- not "4.000").
local function formatCueNo(no)
    local s = string.format('%.3f', no / 1000)
    s = s:gsub('0+$', '')
    s = s:gsub('%.$', '')
    return s
end

-- Executes the actual deletion for every hard-value group a scan
-- found. For each (sequence, cue, part, attribute) group: selects
-- every fixture in that group at once, scopes to just that one
-- attribute, recalls the cue's existing stored value into the
-- programmer (so there is something "active" for /Remove to match -
-- per the manual's own example: "Store Sequence 1 Cue 5 /Remove"
-- removes values active in the programmer AND stored in that cue),
-- then Store .../Remove strips it back out of the cue - leaving every
-- other attribute, every other fixture, and every recipe line on that
-- same part untouched.
--
-- FIRST LIVE ATTEMPT at this technique - not independently verified
-- before this run. Every step is pcall-wrapped and logged rather than
-- trusted blindly, since a Cmd() call in this codebase has repeatedly
-- been seen reporting OK while silently doing nothing - the caller
-- should re-scan afterward and compare the hard-value count, not just
-- trust this log alone.
local function deleteHardGroups(result)
    local log = {}
    local okCount, failCount = 0, 0

    for _, groupKey in ipairs(result.hardGroupOrder) do
        local g = result.hardGroups[groupKey]
        local fixtureList = table.concat(g.fids, '+')
        local cueNoStr = formatCueNo(g.cueNo)

        table.insert(log, string.format(
            '--- %s > %s (cue %s) > Part %d > %s  (%d fixture(s): %s)',
            g.seqName, g.cueName, cueNoStr, g.partIndex, g.attrName, #g.fids, fixtureList))

        local selectCmd = 'Fixture ' .. fixtureList
        local okSel, selErr = pcall(Cmd, selectCmd)
        table.insert(log, string.format('  %s -> ok=%s%s', selectCmd, tostring(okSel), okSel and '' or (' err=' .. tostring(selErr))))

        local attrCmd = string.format('Attribute "%s"', g.attrName)
        local okAttr, attrErr = pcall(Cmd, attrCmd)
        table.insert(log, string.format('  %s -> ok=%s%s', attrCmd, tostring(okAttr), okAttr and '' or (' err=' .. tostring(attrErr))))

        local atCmd = string.format('At %s Cue %s', g.seqAddr, cueNoStr)
        local okAt, atErr = pcall(Cmd, atCmd)
        table.insert(log, string.format('  %s -> ok=%s%s', atCmd, tostring(okAt), okAt and '' or (' err=' .. tostring(atErr))))

        local removeCmd = string.format('Store %s Cue %s Part %d /Remove /NC', g.seqAddr, cueNoStr, g.partIndex)
        local okRemove, removeErr = pcall(Cmd, removeCmd)
        table.insert(log, string.format('  %s -> ok=%s%s', removeCmd, tostring(okRemove), okRemove and '' or (' err=' .. tostring(removeErr))))

        local okClear, clearErr = pcall(Cmd, 'ClearAll')
        table.insert(log, string.format('  ClearAll -> ok=%s%s', tostring(okClear), okClear and '' or (' err=' .. tostring(clearErr))))

        if okSel and okAttr and okAt and okRemove and okClear then
            okCount = okCount + 1
        else
    ...
    return log, okCount, failCount
end

local function writeDeleteLog(log, okCount, failCount)
    local okPath, showsRoot = pcall(function() return GetPath(Enums.PathType.Showfiles) end)
    if not okPath or showsRoot == nil then
        Printf('DeleteHardValues delete-log debug: GetPath(Enums.PathType.Showfiles) failed')
        return nil
    end
    local fullPath = showsRoot .. '/DeleteHardValues_DeleteLog.txt'
    local okOpen, f = pcall(io.open, fullPath, 'w')
    if not okOpen or f == nil then
        Printf('DeleteHardValues delete-log debug: could not open %s for writing', fullPath)
        return nil
    end
    f:write('Delete Hard Values - delete log\n')
    f:write(string.format('Groups attempted: %d (all steps ok: %d, at least one step failed: %d)\n', okCount + failCount, okCount, failCount))
    f:write('NOTE: "ok" only means each Cmd() call did not error - it does NOT\n')
    f:write('confirm the value was actually removed. Re-scan and compare counts.\n')
    f:write(string.rep('-', 70) .. '\n')
    for _, line in ipairs(log) do
        f:write(line .. '\n')
    end
    f:close()
    Printf('DeleteHardValues delete-log debug: wrote %s', fullPath)
    return fullPath
end

local function writeReport(result)
    local okPath, showsRoot = pcall(function() return GetPath(Enums.PathType.Showfiles) end)
    if not okPath or showsRoot == nil then
        Printf('DeleteHardValues report debug: GetPath(Enums.PathType.Showfiles) failed')
        return nil
    end

    local fullPath = showsRoot .. '/DeleteHardValues_Report.txt'
    local okOpen, f = pcall(io.open, fullPath, 'w')
    if not okOpen or f == nil then
        Printf('DeleteHardValues report debug: could not open %s for writing', fullPath)
        return nil
    end

    f:write('Delete Hard Values - scan report\n')
    f:write('Sequences scanned: ' .. table.concat(result.sequencesScanned, ', ') .. '\n')
    f:write(string.format('Cues scanned: %d, cue parts scanned: %d\n', result.cueCount, result.partCount))
    f:write(string.format('Hard values found: %d\n', result.hardCount))
    f:write('This is a SCAN/REPORT ONLY run - nothing was deleted.\n')
    f:write(string.rep('-', 70) .. '\n')
    ...
    if #result.errors > 0 then
        f:write(string.rep('-', 70) .. '\n')
        f:write('Cue parts that could not be read:\n')
        for _, err in ipairs(result.errors) do
            f:write(err .. '\n')
        end
    end
    if #result.debugDumps > 0 then
        f:write(string.rep('=', 70) .. '\n')
        f:write('DIAGNOSTIC: raw data for every cue part with at least 1\n')
        f:write('recipe line (orange "recipe pot" cues), regardless of what\n')
        f:write('the hard-value check above found. Use this to check the\n')
        f:write('detection logic against a cue you know mixes recipes and\n')
        f:write('hard values.\n')
        f:write(string.rep('=', 70) .. '\n')
        for _, d in ipairs(result.debugDumps) do
            f:write(d .. '\n' .. string.rep('-', 40) .. '\n')
        end
    end
    f:close()
    Printf('DeleteHardValues report debug: wrote %s', fullPath)
    return fullPath
end
...
local function Main(display_handle, arguments)
    ...
    local input = TextInput(
        'Which sequence(s) to scan?',
        'name, number, 1 Thru 5, list, or All')
    local scope, parseErr = parseSequenceSelector(input)

    if scope == nil then
        if parseErr == 'cancelled' then
            Printf('Delete Hard Values: cancelled at sequence prompt')
        else
    ...
    Printf('Delete Hard Values: scanning...')
    local result = scanSequences(scope)
    local reportPath = writeReport(result)

    local scopeDesc
    if scope == 'all' then
        scopeDesc = 'All sequences'
    else
        scopeDesc = 'Sequence(s): ' .. table.concat(result.sequencesScanned, ', ')
        if #result.errors > 0 then
            scopeDesc = scopeDesc .. ' (some requested sequences were not found - see report)'
        end
    end

    local reportDesc = reportPath ~= nil
        and ('Report written to:\n' .. reportPath)
        or 'Could not write report file - see System Monitor.'

    Printf('Delete Hard Values: %s | Scanned %d cues / %d parts | Found %d hard value(s)',
        scopeDesc, result.cueCount, result.partCount, result.hardCount)

    if result.hardCount == 0 or #result.hardGroupOrder == 0 then
        local summary = string.format(
            '%s\n\nScanned %d cues / %d cue parts.\nFound %d hard value(s).\n\n%s',
    ...
    -- Ask for confirmation with the count before touching anything -
    -- one popup, no per-item prompts.
    local confirmMsg = string.format(
        '%s\n\nScanned %d cues / %d cue parts.\nFound %d hard value(s) in %d operation(s).\n\n%s\n\nDelete these %d hard value(s) now?\nThis is the first live use of this delete technique - it cannot be fully undone (Oops may still work).',
        scopeDesc, result.cueCount, result.partCount, result.hardCount, #result.hardGroupOrder, reportDesc, result.hardCount)
    ...
    if not okBox or box == nil or box.result ~= 1 then
        Printf('Delete Hard Values: deletion cancelled by user')
        return
    end

    Printf('Delete Hard Values: deleting %d hard value(s) across %d operation(s)...', result.hardCount, #result.hardGroupOrder)
    local log, okCount, failCount = deleteHardGroups(result)
    local logPath = writeDeleteLog(log, okCount, failCount)

    Printf('Delete Hard Values: delete pass done (ok=%d fail=%d), re-scanning to verify...', okCount, failCount)
    local afterResult = scanSequences(scope)
    writeReport(afterResult)

    local resultSummary = string.format(
        'Delete pass complete.\n\nOperations: %d ok, %d had a failed step.\n\nHard values before: %d\nHard values after:  %d\n\n%s\n\n%s',
        okCount, failCount, result.hardCount, afterResult.hardCount,
        logPath ~= nil and ('Delete log:\n' .. logPath) or 'Could not write delete log - see System Monitor.',
        'Verify the after-count and check a few cues directly before trusting this on anything but a test show.')
